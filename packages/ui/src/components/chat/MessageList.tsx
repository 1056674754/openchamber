...
import { filterSyntheticParts, isFullySyntheticMessage } from '@/lib/messages/synthetic';
...
import { isStaticTool } from './message/parts/toolRenderUtils';
...
import { extractTextContent, normalizeParts } from './message/partUtils';
...
const setClientRole = (message: ChatMessageEntry, role: string): ChatMessageEntry => ({
    ...message,
    info: {
        ...(message.info as unknown as Record<string, unknown>),
        clientRole: role,
...
const isAutoGeneratedUserMessage = (message: ChatMessageEntry): boolean => {
    if (resolveMessageRole(message) !== 'user') return false;
    const parts = Array.isArray(message.parts) ? message.parts : [];
    if (isSystemDirectiveMessage(parts)) return true;
...
const normalizeAutoGeneratedUserMessage = (message: ChatMessageEntry): ChatMessageEntry => {
    if (!isAutoGeneratedUserMessage(message)) return message;
    return setClientRole(message, 'assistant');
};
...
const getNormalizedMessageForDisplay = (message: ChatMessageEntry): ChatMessageEntry => {
    ...
    const roleNormalized = normalizeAutoGeneratedUserMessage(normalized);

    normalizedMessageBySource.set(message, roleNormalized);
...
const TurnBlock = React.memo(({
    ...
    const isInvisibleBoundary = (p: Part): boolean =>
        p.type === 'step-start' || p.type === 'step-finish' || p.type === 'snapshot';

    const crossMessageChain = React.useMemo(() => {
        if (chatRenderMode !== 'live') return { absorbedIds: new Map<string, Set<string>>(), extraTrailingById: new Map<string, ToolPartType[]>() };

        const getVisible = (parts: Part[]): Part[] =>
            parts.filter(p => !isEmptyTextPart(p) && (p as Record<string, unknown>).type !== 'compaction');

        const getLeading = (parts: Part[]): ToolPartType[] => {
            const result: ToolPartType[] = [];
            for (const p of getVisible(parts)) {
                if (p.type === 'tool' && isStaticTool((p as ToolPartType).tool)) {
                    result.push(p as ToolPartType);
                } else if (isInvisibleBoundary(p)) {
                    continue;
                } else { break; }
            }
            return result;
        ...
        const getTrailing = (parts: Part[]): ToolPartType[] => {
            const visible = getVisible(parts);
            const result: ToolPartType[] = [];
            for (let k = visible.length - 1; k >= 0; k--) {
                const p = visible[k];
                if (p.type === 'tool' && isStaticTool((p as ToolPartType).tool)) {
                    result.unshift(p as ToolPartType);
                } else if (isInvisibleBoundary(p)) {
                    continue;
                } else { break; }
            }
        ...
        const absorbedIds = new Map<string, Set<string>>();
        const extraTrailingById = new Map<string, ToolPartType[]>();

        for (let idx = 1; idx < visibleAssistantMessages.length; idx++) {
            const prevMsg = visibleAssistantMessages[idx - 1];
            const currMsg = visibleAssistantMessages[idx];
            const prevTrailing = getTrailing(prevMsg.parts as Part[]);
            const currLeading = getLeading(currMsg.parts as Part[]);

            if (prevTrailing.length > 0 && currLeading.length > 0) {
                const prevId = prevMsg.info.id;
                const currId = currMsg.info.id;
                absorbedIds.set(currId, new Set(currLeading.map(t => t.id)));
                const existing = extraTrailingById.get(prevId) ?? [];
                extraTrailingById.set(prevId, [...existing, ...currLeading]);
            }
        }

        return { absorbedIds, extraTrailingById };
    }, [chatRenderMode, visibleAssistantMessages]);
        ...
        (message: ChatMessageEntry) => {
            ...
            const turnGroupingContext = isAssistantMessage
                ? {
                    ...
                    absorbedToolPartIds: crossMessageChain.absorbedIds.get(message.info.id),
                    extraTrailingTools: crossMessageChain.extraTrailingById.get(message.info.id),
